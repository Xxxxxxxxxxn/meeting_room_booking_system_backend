...
export class UserService {
  ...
  async updatePassword(passwordDto: UpdateUserPasswordDto) {
    ...
    if (!captcha) {
      throw new HttpException('缺少验证码', HttpStatus.BAD_REQUEST);
    }
    ...
    if (userInfo.email !== passwordDto.email) {
      throw new HttpException('邮箱有误', HttpStatus.BAD_REQUEST);
    }

    userInfo.password = md5(passwordDto.password);
  ...
  async frozen(id: number, isFrozen: string) {
    const userInfo = await this.userRepository.findOneBy({ id });
    if (!userInfo) {
      throw new HttpException('不存在该用户', HttpStatus.BAD_REQUEST);
    }

    userInfo.isFrozen = isFrozen === '0' ? false : true;
